...
1. **기준 위험률 표**: Claude가 강의용 **합성 더미 표**를 생성한다 — 사망률(남·여, 연령 0~100, 연령에 따라 단조 증가하는 합성 값)과 특약용 진단률 표. 생성 규칙(산식·파라미터)을 표와 함께 `docs/domain/golden-cases.md`에 기록하고, 공용 라이브러리 시드 데이터로도 사용한다.
...
| 2 | 골든 기준값 | **더미 표 + 이중 산출**: Claude가 합성 더미 위험률 표(사망률·진단률, 연령 0~100)를 생성하고, 앱 엔진과 완전히 독립된 별도 Python 스크립트로 기대값을 이중 산출해 고정. 두 경로 일치가 통과 조건. 사용자는 최종 수치만 검수 | §7.3 |
