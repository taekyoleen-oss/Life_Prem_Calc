...
> **개정 이력**: v1.0 초안 → v1.1 리뷰 6건 확정 반영(상세 §10) → v1.2 구현 착수 전 2차 리뷰 4건 확정 반영: 페이즈별 체크포인트 빌드, 골든 기준값 더미 표 + 이중 산출, 게스트 우선 개발(Supabase는 P5 연동), 매직링크 우선 인증. 상세는 §11.
...
- v1.0 인증은 **이메일 매직링크만** 구현하고, Google OAuth는 코드상 자리만 마련해 두었다가 콘솔 설정 완료 후 활성화한다. (v1.2 확정, §2.4)
...
| `/login` | 로그인 | 이메일 매직링크(Supabase Auth). Google OAuth는 콘솔 설정 후 활성화(§2.4) | 불필요 |
...
**인증 수단(v1.2 확정)**: v1.0은 **이메일 매직링크만** 구현한다. Google OAuth는 Supabase provider 설정과 로그인 버튼 자리만 코드에 마련해 두고(기본 비활성), Google Cloud Console에서 OAuth 클라이언트 생성·설정(사용자 작업)이 완료된 뒤 활성화한다. 학생용 구글 계정 로그인 편의는 이 활성화 시점부터 제공된다.
...
- Supabase Auth: 이메일 매직링크(v1.0 기본). Google OAuth는 코드상 자리만 마련해 두고 콘솔 설정 완료 후 활성화한다(§2.4, v1.2 확정).
...
| `NEXT_PUBLIC_APP_URL` | 매직링크 리다이렉트 기준 URL | P5부터 |
...
| P5 저장·동기화·QA·배포 | localAdapter·cloudAdapter, Supabase 프로젝트 생성·연동(Auth 매직링크·RLS), 게스트 가져오기, 라이브러리 페이지, Vercel 배포, 접근성·성능 마감 | 게스트 → 로그인 이전 무손실, Lighthouse 성능·접근성 ≥ 90, 키보드 단계 이동, 배포 URL 정상 동작 | 규칙 기반 + 사람 검토 | 배포 URL 접속·게스트 가져오기 확인(v1.0 완료 승인) | 선택 기능은 단순 버전 폴백 + 로그 |
...
| 4 | 로그인 방식 | **매직링크 우선**: v1.0은 이메일 매직링크만 구현. Google OAuth는 코드상 자리만 마련하고 Google Cloud Console 설정(사용자 작업) 완료 후 활성화(v1.x) | §2.1, §2.4, §4.3, §8.1 |
