# PremiaFlow : 생명보험 보험료 단계별 산출 웹앱 설계서 v1.2
...
> **작성일**: 2026-08-27 · **최종 수정**: 2026-08-28 · **상태**: v1.2 확정(2차 리뷰 반영 완료, 구현 착수 가능)
> **개정 이력**: v1.0 초안 → v1.1 리뷰 6건 확정 반영(상세 §10) → v1.2 구현 착수 전 2차 리뷰 4건 확정 반영: 페이즈별 체크포인트 빌드, 골든 기준값 더미 표 + 이중 산출, 게스트 우선 개발(Supabase는 P5 연동), 매직링크 우선 인증(→ v1.0 구현 후 이메일+비밀번호로 변경, §2.4). 상세는 §11.
...
**기본 해석과 확정 사항** (리뷰 확정 이력은 §10·§11 참조)
...
- 개발은 **게스트(로컬) 모드 우선**으로 진행한다: P0~P4는 localStorage 기반으로 전체 기능을 완성·검증하고, Supabase 프로젝트 생성·연동과 Vercel 배포는 P5에서 수행한다. (v1.2 확정, §7.1)
- v1.0 인증은 **이메일 + 비밀번호**로 구현하고(v1.2 매직링크 → 사용자 요청으로 변경), Google OAuth는 코드상 자리만 마련해 두었다가 콘솔 설정 완료 후 활성화한다. (§2.4)
...
| `/login` | 로그인·회원가입 | 이메일 + 비밀번호(Supabase Auth). 비밀번호 재설정 포함. Google OAuth는 콘솔 설정 후 활성화(§2.4) | 불필요 |
| `/workbooks` | 워크북 목록 | 생성·복제·삭제, 최근 수정순 | 필요 |
| `/w/[workbookId]` | (v1.0 미사용) | 작업공간을 `/w/guest` 하나로 통합했다 — 클라우드 워크북은 `/workbooks`에서 불러와 같은 화면에서 편집한다. 이 경로는 `/workbooks`로 리다이렉트하며, 워크북별 딥링크는 v1.x 이연 | 불필요 |
| `/w/guest` | 작업공간(본체) | 시트 탭 + 단계형 파이프라인 + 결과 패널. 게스트는 localStorage, 클라우드 연결 시 2초 디바운스로 자동 저장 | 불필요 |
...
- 대표 구성은 **주계약 + 특약**이다: 공용탭에 계약조건과 사망률을 두고, 주계약 탭은 이를 그대로 참조하며, 특약 탭은 공용탭의 사망률에 탭 로컬로 업로드한 특약 발생률(암발생률 등)을 결합해 산출한다.
...
**인증 수단**: v1.0은 **이메일 + 비밀번호**로 구현한다. 한 번 가입하면(메일 확인 없이 즉시 완료 — `enable_confirmations = false`) 이후에는 비밀번호로 로그인하고, 세션은 브라우저에 유지·자동 갱신된다. 비밀번호 재설정만 메일 링크를 쓰며 `/login?reset=1`에서 새 비밀번호를 설정한다. Google OAuth는 Supabase provider 설정과 로그인 버튼 자리만 코드에 마련해 두고(기본 비활성), Google Cloud Console에서 OAuth 클라이언트 생성·설정(사용자 작업)이 완료된 뒤 활성화한다. 학생용 구글 계정 로그인 편의는 이 활성화 시점부터 제공된다.
...
| M03 | 위험률 표 | ○ | 파일 업로드(CSV·XLSX)·클립보드 붙여넣기·라이브러리 선택 | 컬럼 매핑(첫 열 = 연령 자동 감지), 담보 다중 선택(성별 구분 없음 — 성별은 M02), **사망률 여부 플래그**(라이브러리 메타에서 자동 제안) | 계열 `q1`, `q2`… |
...
|-----------|-------------|---------------------|
| 위험률 계열 | `q1`, `q2`… | q_일반사망 (라이브러리 메타에서 생성) |
| 현가율 계열 | `v1`… | v_2.50% |
...
- **정합성 보장**: 골든 케이스에 대해 생성된 Python을 실제 실행하여 엔진 결과와 대사하는 자동 테스트를 CI에 포함한다(§7.3). 사용자 수식(M10)도 동일 AST에서 Python·VBA 표현식으로 변환한다. VBA는 CI에서 자동 실행이 불가하므로(Excel 필요) 골든 케이스에 대한 수동 스팟 체크로 검증한다(v1.2 확정).
...
- Supabase Auth: 이메일 + 비밀번호(v1.0 기본, `signUp`/`signInWithPassword`/`resetPasswordForEmail`). Google OAuth는 코드상 자리만 마련해 두고 콘솔 설정 완료 후 활성화한다(§2.4, v1.2 확정).
...
- **개발 순서(v1.2 확정)**: P0~P4는 `localAdapter`(게스트 모드)만으로 전체 기능을 개발·검증한다. Supabase 프로젝트 생성, `cloudAdapter`·Auth·RLS 구현, Vercel 배포는 P5에서 수행한다(§7.1). 어댑터 인터페이스는 P0 타입 골격 단계에서 먼저 확정하여 P5 전환 비용을 최소화한다.
...
- 프로젝트 루트는 현재 폴더 `C:\00 App Project\Life_Prem_Calc`를 그대로 사용한다(위 트리의 `/premiaflow` = 현재 폴더). P0에서 `git init`으로 로컬 저장소를 만든다.
- 개발 환경: Node 22 · npm 10.
- 테스트 스택: **vitest**(엔진·수식 파서 단위 테스트, 골든 테스트), **pytest**(생성 Python 대사 테스트). UI 언어는 한국어 전용.
...
| 변수명 | 용도 | 필요 시점 |
|--------|------|-----------|
| `NEXT_PUBLIC_SUPABASE_URL` | Supabase 프로젝트 URL | P5(클라우드 연동)부터 |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | Supabase 공개 키 | P5부터 |
| `SUPABASE_SERVICE_ROLE_KEY` | 관리 스크립트·시드 전용(서버 한정) | P5부터 |
| `NEXT_PUBLIC_APP_URL` | 인증 메일(비밀번호 재설정) 리다이렉트 기준 URL | P5부터 |
| `ANTHROPIC_API_KEY` | v2.0 산출방법서 파싱 전용(v1.0 미사용) | v2.0 |

P0~P4(게스트 모드 개발)에는 환경 변수가 전혀 필요 없다. `.env` 파일은 git에 커밋하지 않는다.
...
정확도 최우선 원칙에 따라 **UI보다 계산 엔진과 골든 테스트를 먼저** 구축한다. 진행 방식은 **페이즈별 체크포인트**(v1.2 확정)로, 아래 표의 사용자 체크포인트가 있는 페이즈는 사용자 확인 후 다음 페이즈로 진행한다. Supabase 연동·Vercel 배포는 P5로 이동했다(v1.2 확정).

| Phase | 산출물 | 성공 기준 | 검증 방법 | 사용자 체크포인트 | 실패 시 처리 |
|-------|--------|-----------|-----------|--------------------|---------------|
| P0 스캐폴딩 | Next.js 15 + TweakCN 테마 + 라우팅 골격 + 타입 골격(파이프라인 스키마·저장 어댑터 인터페이스) + git init | 빌드·타입 오류 0 | 규칙 기반 | 없음(자율 진행) | 자동 재시도(최대 3회) |
| P1 엔진·골든 | `lib/engine`, `lib/formula`, 더미 위험률 표, 골든 케이스 G1~G4 | G1~G4 전부 통과(독립 Python 이중 산출값과 일치), 핵심 함수 단위 테스트 존재 | 규칙 기반(vitest) + 타입 | 골든 테스트 통과 리포트·기준 수치 검수 | 재시도 3회 실패 시 산식 가정 확인으로 에스컬레이션 |
| P2 핵심 파이프라인 UI | 스텝퍼 + M01~M08 + DataGrid + 결과 패널 | 정기보험 1건을 화면 입력만으로 산출(E2E), 그리드 TSV 복사 동작 | LLM 자기 검증 + 사람 검토(브라우저 확인) | 브라우저에서 정기보험 1건 산출 확인 | 가상 스크롤 문제 시 페이지네이션 폴백 + 로그 |
| P3 자산·탭·옵션 | 공용탭 자동 생성·참조, 시트 탭·복제, M05 다중 인스턴스(사망 플래그 구분), 현가 시점 옵션, M09 | 연중 현가·독립 곱 옵션 반영값이 골든 변형 케이스와 일치, 공용탭 자산 수정 시 참조 탭 전체 재계산 동작, 특약 탭 골든 케이스 G5 일치 | 규칙 기반 + 골든 | 없음(자율 진행) | 자동 재시도 |
| P4 수식 빌더·코드 생성·강의 모드 | M10, CodePanel, `lib/codegen`, content 설명 초안 | 생성 Python 실행값 = 엔진값(G1~G5 대사 테스트 통과, 공용탭 참조 포함), 수식 순환 참조 차단 | 규칙 기반(pytest 대사) + 사람 검토(설명 문안) | Python 대사 결과·설명 문안 확인(VBA는 수동 스팟 체크) | 템플릿 규약 모호 시 에스컬레이션 |
| P5 저장·동기화·QA·배포 | localAdapter·cloudAdapter, Supabase 프로젝트 생성·연동(Auth 이메일+비밀번호·RLS), 게스트 가져오기, 라이브러리 페이지, Vercel 배포, 접근성·성능 마감 | 게스트 → 로그인 이전 무손실, Lighthouse 성능·접근성 ≥ 90, 키보드 단계 이동, 배포 URL 정상 동작 | 규칙 기반 + 사람 검토 | 배포 URL 접속·게스트 가져오기 확인(v1.0 완료 승인) | 선택 기능은 단순 버전 폴백 + 로그 |

**구현 착수 전 사용자 선행 작업: 0건.** 게스트 우선 개발 + 매직링크 우선 인증 + 더미 위험률 표 조합이므로 계정·키·위험률 표 준비 없이 즉시 P0에 착수할 수 있다. 사용자 작업이 처음 필요한 시점은 P5의 Supabase·Vercel 계정 연결이다.

### 7.2 프로젝트 전체 성공 기준

강의 예제 3건(정기보험·생사혼합·주계약+진단특약 구성)을 처음 쓰는 사용자가 각 10분 이내에 입력부터 보험료 산출까지 재현할 수 있고, 각 단계에서 내려받은 Python 스크립트의 실행 결과가 화면 값과 완전히 일치하면 v1.0 완성으로 본다. 특약 예제에서는 공용탭에 입력한 사망률을 주계약·특약 탭이 함께 참조하는 흐름까지 포함한다. 위 기준은 로컬이 아닌 **Vercel 배포 URL에서 동일하게 동작하는 것**까지 포함한다(v1.2 확정).
...
| G5 | G1을 공용탭(계약조건·일반사망률·이자율)으로 옮기고 특약 탭 추가: 암발생률(더미 표, 사망률 플래그 없음) × 공용탭 일반사망률의 독립 곱으로 l_특약 산출, 암진단급부 S'=1천만 | 특약 연납 P, l_특약·d_진단 계열, 공용탭 사망률 수정 시 주계약·특약 동시 재계산 |

**기대값 확정 절차(v1.2 확정: 더미 표 + 이중 산출)**

1. **기준 위험률 표**: Claude가 강의용 **합성 더미 표**를 생성한다 — 연령 0~100에서 단조 증가하는 담보별 합성 표 5종(일반사망률·재해사망률·50% 이상 장애율·암발생률·암수술률). 성별 구분은 두지 않는다(성별은 M02 계약조건). 생성 규칙(산식·파라미터)을 표와 함께 `docs/domain/golden-cases.md`에 기록하고, 공용 라이브러리 시드 데이터로도 사용한다.
2. **이중 산출**: 기대값은 앱 엔진(TypeScript)과 **완전히 독립된 별도 Python 스크립트**로 산출한다. 이 스크립트는 코드 생성기 템플릿과도 코드·로직을 공유하지 않는 독립 경로여야 한다.
3. **통과 조건**: 엔진 결과와 독립 산출값의 완전 일치(float64 기준). 확정된 값은 `docs/domain/golden-cases.md`에 고정한다.
4. **사용자 검수**: 사용자(강의자)는 P1 체크포인트에서 최종 수치(NSP·연납 P·G 등)의 계리적 타당성만 검수한다. 계산 과정 검증은 이중 산출이 담당한다.
5. 이후 모든 엔진·코드 생성 변경은 이 고정값과의 완전 일치를 통과 조건으로 한다. 실제 경험생명표 기반 검증은 사용자가 표를 업로드하는 시점에 별도로 수행할 수 있다.
...
- Google OAuth 활성화(콘솔 설정 완료 시점, v1.x)
...
## 11. 2차 리뷰 확정 이력 (2026-08-28)

구현 착수 전 객관식 인터뷰 4건에 대한 사용자 답변과 반영 결과. 4건 모두 추천안이 채택되었다.

| # | 항목 | 확정 결과 | 반영 위치 |
|---|------|-----------|-----------|
| 1 | 빌드 방식 | **페이즈별 체크포인트**: P1(골든 테스트 통과 리포트·기준 수치 검수), P2(브라우저에서 정기보험 1건 산출 확인), P4(Python 대사 결과·설명 문안 확인), P5(배포 URL 확인 = v1.0 완료 승인) 시점에 사용자 확인 후 다음 페이즈 진행. P0·P3은 자율 진행 | §7.1 |
| 2 | 골든 기준값 | **더미 표 + 이중 산출**: Claude가 합성 더미 위험률 표(담보 5종, 연령 0~100, 성별 구분 없음)를 생성하고, 앱 엔진과 완전히 독립된 별도 Python 스크립트로 기대값을 이중 산출해 고정. 두 경로 일치가 통과 조건. 사용자는 최종 수치만 검수 | §7.3 |
| 3 | DB 연동 시점 | **게스트 우선, P5에서 연동**: localStorage(게스트) 모드로 P0~P4 전체 기능을 완성한 뒤 P5에서 Supabase 신규 프로젝트 생성·연동. 저장 어댑터 인터페이스는 P0에서 먼저 확정 | §4.3, §6.5, §7.1 |
| 4 | 로그인 방식 | **이메일 + 비밀번호**(v1.2 매직링크 우선 → 구현 후 사용자 요청으로 변경): 한 번 가입 후 비밀번호 로그인. Google OAuth는 코드상 자리만 마련하고 Google Cloud Console 설정(사용자 작업) 완료 후 활성화(v1.x) | §2.1, §2.4, §4.3, §8.1 |

**기본 처리 사항** (상투적 결정 — 인터뷰 생략, 기본값 적용)

- **프로젝트 루트**: 현재 폴더 `Life_Prem_Calc`를 그대로 프로젝트 루트로 사용, P0에서 `git init` (§6.1)
- **UI 언어**: 한국어 전용 (§6.1)
- **VBA 검증**: CI 자동 실행이 불가(Excel 필요)하므로 골든 케이스 수동 스팟 체크. Python만 CI 대사 테스트 (§3.5, §7.1 P4)
- **배포**: Vercel 배포는 P5에서 Supabase 연동과 함께 수행하고, v1.0 완성 기준에 배포 URL 동작을 포함 (§7.1, §7.2)
- **구현 착수 전 사용자 선행 작업 0건**: 게스트 우선 + 매직링크 우선 + 더미 표 조합으로 계정·키·위험률 표 준비 없이 즉시 P0 착수 가능 (§7.1)
